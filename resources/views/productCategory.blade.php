...
@section('content')
    ...
    <div class="container mx-auto py-12">
        <h2 class="text-xl font-bold mb-6">Our Products for category "{{$category -> name}}"</h2>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            @foreach ($products as $product)
                <a href="{{ route('viewproduct', $product->id) }}"
                    ...
                    @endif
                    <h3 class="text-slate-700 text-xs">{{ $product->brand->name }}</h3>
                    <h3 class="text-slate-700 text-xs">{{ $product->category->name }}</h3>
                </a>
            @endforeach
        </div>
