...
@section('content')
            ...
            <table class="table table-fixed " id="mytable">
                ...
                <tbody>
                    @forelse ($carts as $cart)
                        <tr>
                            ...
                            <td><a class="bg-red-500 text-white rounded px-2 py-1 text-xs m-2"
                                    href="{{ route('carts.delete', $cart->id) }}">Remove</a></td>
                ...
                <tfoot>
                    <tr scope="row">
                        ...
                        <th>{{ $grandTotal }}</th>
                        <th><button class="bg-blue-500 w-full text-white rounded px-2 py-2 text-xs m-2"
                                onclick="showcheckout()">Checkout</button></th>
                    </tr>
    ...
    <div id="checkout" class="hidden  backdrop-blur-md fixed top-0 left-0 h-screen w-screen  justify-center items-center ">
        ...
        <div class="bg-white p-5 rounded shadow-xl relative ">
            <button class="absolute top-1 right-2 hover:rounded hover:bg-slate-200 p-2" onclick="showcheckout()"> <i
            ...
            <form action="{{ route('order.store') }}" method="POST">
                ...
                <div class="m-2 ">
                    @if ($errors->all())
                        @foreach ($errors->all() as $error)
                            <p class="bg-red-400 text-white my-2  px-4 py-1 rounded-lg shadow text-lg font-thin">
                                {{ $error }}</p>
                        @endforeach
                    @endif
                ...
                <div>
                    ...
                    <button class="w-full bg-violet-500 text-white rounded m-2 text-sm py-2" id="payment-button"
                        type="button">Pay with Khalti</button>
                </div>
        </div>
        </form>
